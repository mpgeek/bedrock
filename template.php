 ...
 *   The theme of interest; usually the active theme.
 */
function bedrock_get_css_files($theme_name) {
  $styles = &drupal_static(__FUNCTION__, array());
  if (empty($styles)) {
    if ($cache = cache_get('bedrock_get_css_files')) {
      $styles = $cache->data;
    }
    else {
      ...
      $info_data = bedrock_get_info_trail($theme_name);
      ...
      cache_set('bedrock_get_css_files', $styles, 'cache');
    }
  }
...
function bedrock_load_polyfills($theme_name) {
  ...
  $polyfills_array = drupal_static('bedrock_preprocess_html_polyfills_array');
  if (empty($polyfills_array)) {
    ...
    if (array_key_exists('ie_scripts', $info)) {
      foreach ($info['ie_scripts'] as $condition => $ie_scripts_path) {
        foreach ($ie_scripts_path as $key => $value) {
          ...
          $polyfills_array['info'][$condition][] = bedrock_theme_conditional_script($filepath);
        }
      }
    }
    // Bedrock Core Polyfills
    $polly = '';
    ...
    foreach ($backed_crackers as $cupboard => $flavors) {
      foreach ($flavors as $key => $value) {
        ...
        $polyfills_array['bedrock'][$cupboard][] = bedrock_theme_conditional_script($filepath);
      }
    }
  }
...
function bedrock_load_debuggers($theme_name) {
  ...
  if (bedrock_get_setting('expose_regions', $theme_name)) {
    drupal_add_css($path_to_bedrock . '/css/debug-regions.css');
  }

  if (bedrock_get_setting('load_all_panels', $theme_name)) {
    drupal_add_css($path_to_bedrock . '/css/debug-panels.css', array(
      'preprocess' => FALSE,
      'group' => CSS_THEME,
      'media' => 'screen',
      'every_page' => FALSE,
  ...
  if (bedrock_get_setting('show_window_size', $theme_name)) {
    drupal_add_js($path_to_bedrock . '/scripts/window-size.js', array(
      'type' => 'file',
      'scope' => 'footer',
      'group' => JS_THEME,
      'preprocess' => TRUE,
 ...
 *
 * This is almost always called from bedrock_preprocess_html(), only in
 ...
 *   The media query from theme settings.
 * @param string $theme_name
 *   The theme of interest; usually the active theme.
 * @param number $weight
 *   Optional.
 ...
 * @param $filepath, path to the file.
 */
function bedrock_theme_conditional_script($filepath) {
  $script = '';
 ...
 */
function bedrock_theme_conditional_scripts($ie_scripts) {
      ...
      if (!empty($crackers)) {
        ...
        if (isset($crackers['all'])) {
          foreach ($crackers['all'] as $script) {
            drupal_add_js($path_to_bedrock . '/' . $script, array(
              'type' => 'file',
              'scope' => 'header',
              'group' => JS_THEME,
              'preprocess' => TRUE,
...
function bedrock_process_html(&$vars) {
  ...
  if (!empty($vars['polyfills_array'])) {
    $vars['polyfills'] = drupal_static('bedrock_process_html_polyfills');
    if (empty($vars['polyfills'])) {
      ...
      $vars['polyfills'] = bedrock_theme_conditional_scripts($aus);
    }
  }
  else {
    $vars['polyfills'] = '';
 ...
 */
function bedrock_process_maintenance_page(&$vars) {
